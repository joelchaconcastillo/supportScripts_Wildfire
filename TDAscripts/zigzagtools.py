...
def shift_filtration(rips,n):
    ...
    f = d.Filtration()
    for s in rips:  #go through each simplex...
        dim = s.dimension() #simplex dim
        temp = []
...
def build_zigzag_times(rips,n,numbins):
    ...
    i=0
    for x in rips:
       dim = x.dimension() # dim of this simplex
       t = [];
       for k in range(0,dim+1): #0 or 1 holes, data is time..
          t.append(x[k])   #vertices
       ...
       elif xmax > xmin:  # When does this happen?
